 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (version_major == 2) {  // MMAV2
      ...
      auto new_a_type = RankedTensorType::get(
          a_type.getShape(), a_type.getElementType(), new_a_encoding);
      a = rewriter.create<ConvertLayoutOp>(a.getLoc(), new_a_type, a);

      // convert B operand
      auto b_type = cast<RankedTensorType>(b.getType());
      auto new_b_encoding =
          DotOperandEncodingAttr::get(context, 1, mma_enc, k_width);
      auto new_b_type = RankedTensorType::get(
          b_type.getShape(), b_type.getElementType(), new_b_encoding);
      b = rewriter.create<ConvertLayoutOp>(b.getLoc(), new_b_type, b);
    ...
    auto new_meta_type = RankedTensorType::get(
        meta_type.getShape(), meta_type.getElementType(),
        triton::gpu::SparseDotMetaEncodingAttr::get(context, mma_enc));
    meta = rewriter.create<ConvertLayoutOp>(meta.getLoc(), new_meta_type, meta);
