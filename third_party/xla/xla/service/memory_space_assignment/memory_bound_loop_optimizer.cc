...
void LoopOptimizerBestFitHeap::CreateBufferInterval(
    LoopOptimizerAllocation* allocation_block,
    LoopOptimizerAllocation* colocated_with) {
  buffer_intervals_[allocation_block] = BufferInterval(
      allocation_block, allocation_block->size, allocation_block->start_time,
...
LoopOptimizerBestFitHeap::MaybeFindChunkCandidate(
    const LoopOptimizerAllocation* allocation_block, int64_t preferred_offset) {
...
void LoopOptimizerBestFitHeap::CommitChunkForAllocation(
    const LoopOptimizerAllocation* allocation_block, const Chunk& chunk) {
...
LoopOptimizerAllocation*
LoopOptimizerBestFitHeap::CreateEvenLoopOptimizerAllocation(int64_t begin_idx,
                                                            int64_t end_idx,
                                                            int64_t size) {
  allocation_blocks_.push_back(std::make_unique<LoopOptimizerAllocation>(
      allocation_blocks_.size(), begin_idx, end_idx, size));
  LoopOptimizerAllocation* first_allocation_block =
      allocation_blocks_.back().get();
  CreateBufferInterval(first_allocation_block);
  allocation_blocks_.push_back(std::make_unique<LoopOptimizerAllocation>(
      allocation_blocks_.size(), begin_idx + 2 * loop_size_,
      end_idx + 2 * loop_size_, size));
  LoopOptimizerAllocation* second_allocation_block =
      allocation_blocks_.back().get();
...
LoopOptimizerAllocation*
LoopOptimizerBestFitHeap::CreateOddLoopOptimizerAllocation(int64_t begin_idx,
                                                           int64_t end_idx,
                                                           int64_t size) {
  allocation_blocks_.push_back(std::make_unique<LoopOptimizerAllocation>(
      allocation_blocks_.size(), begin_idx + loop_size_, end_idx + loop_size_,
      size));
  LoopOptimizerAllocation* allocation_block = allocation_blocks_.back().get();
  CreateBufferInterval(allocation_block);
  return allocation_block;
}

std::pair<LoopOptimizerAllocation*, LoopOptimizerAllocation*>
LoopOptimizerBestFitHeap::CreateEvenAndOddLoopOptimizerAllocation(
    int64_t begin_idx, int64_t end_idx, int64_t size) {
  return std::make_pair(
      CreateEvenLoopOptimizerAllocation(begin_idx, end_idx, size),
      CreateOddLoopOptimizerAllocation(begin_idx, end_idx, size));
}
...
LoopValueChunks LoopOptimizerBestFitHeap::AllocateEvenAndOddBetween(
  ...
  auto [even_allocation, odd_allocation] =
      CreateEvenAndOddLoopOptimizerAllocation(begin_idx, end_idx, size);
...
std::string LoopOptimizerBestFitHeap::MemoryUsageToAsciiArt(
  ...
  absl::StrAppend(&memory_usage_str,
                  interval_tree_.NodesOverlappingInTimeToAsciiArt(
                      0, loop_size_ * 4 - 1, loop_size_));
...
MemoryBoundLoopOptimizer::Create(
  ...
  std::unique_ptr<MemoryBoundLoopOptimizer> optimizer =
      absl::WrapUnique(new MemoryBoundLoopOptimizer(
          ...
          reserved_scoped_memory_fn, alignment_in_bytes));
