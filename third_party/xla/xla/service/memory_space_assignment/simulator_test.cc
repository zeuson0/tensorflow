...
class MemorySpaceAssignmentSimulatorTest : public HloTestBase {
 protected:
  absl::Status Initialize(const HloModule* module) {
    ...
    // Assume 1 byte per second for testing.
    _options.alternate_mem_bandwidth_bytes_per_second = 1;
    TF_ASSIGN_OR_RETURN(
        cost_analysis_,
...
TEST_F(MemorySpaceAssignmentSimulatorTest, SingleLayerNestedLoop) {
  ...
  // The while loop has 42 iterations
  // overhead for instruction:
  // %tuple.anon: 8 sec * 1
  // %greater: 9 sec * 42
  // %increment: 12 sec * 42
  // %loop_result: 8 sec * 42
  // summation: 8 * 1 + 9 * 42 + 12 * 42 + 8 * 42 = 1226
  // NOTE: all overhead are caused by accessing operands/outputs from/to default
  // memory.
  float expected_elapsed_time = 1226;
  ...
  EXPECT_EQ(runtime_simulator_->SimulateElapsedTime(
                ...
                /*kAlternateMemorySpace=*/1,
                /*kDefaultMemoryBandwidth=*/1),
            expected_elapsed_time);
}
...
TEST_F(MemorySpaceAssignmentSimulatorTest, SingleAsyncCopyOverhead) {
  ...
  EXPECT_EQ(runtime_simulator_->SimulateElapsedTime(
                module.get(), *hlo_live_range, allocations,
                /*alternate_memory_space=*/1,
                /*default_memory_bytes_per_second=*/1),
            expected_elapsed_time);
}
...
TEST_F(MemorySpaceAssignmentSimulatorTest, AsyncCopyWithComputationOverhead) {
  ...
  // Pass an empty allocation sequence for test.
  memory_space_assignment::AllocationSequence allocations;

  // Workflow:
  // neg_compute: | read 8 bytes | write 8 bytes |
  // copy-done.1:                | read 8 bytes  | read 24 bytes |
  // time:        |     8 sec    |     16 sec    |     24 sec    |
  // Bandwidth:   |  read only   | read-write shared|   read only   |
  // Note: the compute overhead in neg_compute is overlapped with read/write
  // process
  float expected_elapsed_time = 48;
  EXPECT_EQ(runtime_simulator_->SimulateElapsedTime(
                module.get(), *hlo_live_range, allocations,
                /*alternate_memory_space=*/1,
                /*default_memory_bytes_per_second=*/1),
