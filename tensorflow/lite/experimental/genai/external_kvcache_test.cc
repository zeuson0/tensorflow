...
class ExternalKVSingleOpModel : public SingleOpModel {
 public:
  ExternalKVSingleOpModel(const TensorData& k_cache, const TensorData& v_cache,
    ...
    SetCustomOp("KV_Cache", {}, ops::custom::Register_EXTERNAL_KV_CACHE);
  ...
  TfLiteStatus Run(const std::vector<int64_t>& position,
                   const std::vector<float>& k_sclie,
                   const std::vector<float>& v_sclie) {
    ...
    PopulateTensor(k_slice_, k_sclie);
    PopulateTensor(v_slice_, v_sclie);
...
TEST_P(EKVCacheTest, SingleSliceUpdateTest) {
  ...
  {
    ASSERT_EQ(m.Run(/*position=*/{0}, /*k_sclie=*/{10, 11, 12, 13},
                    /*v_sclie=*/{20, 21, 22, 23}),
              kTfLiteOk);
  ...
  {
    ASSERT_EQ(m.Run(/*position=*/{2}, /*k_sclie=*/{50, 51, 52, 53},
                    /*v_sclie=*/{60, 61, 62, 63}),
              kTfLiteOk);
  ...
  {
    ASSERT_EQ(m.Run(/*position=*/{1}, /*k_sclie=*/{70, 71, 72, 73},
                    /*v_sclie=*/{80, 81, 82, 83}),
              kTfLiteOk);
  ...
  {
    ASSERT_EQ(m.Run(/*position=*/{1}, /*k_sclie=*/{1, 2, 3, 4},
                    /*v_sclie=*/{1, 2, 3, 4}),
              kTfLiteOk);
...
TEST_P(EKVCacheTest, MultipleSliceUpdateTest) {
  ...
  {
    ASSERT_EQ(m.Run(/*position=*/{0, 1}, /*k_sclie=*/{1, 1, 1, 1, 2, 2, 2, 2},
                    /*v_sclie=*/{5, 5, 5, 5, 6, 6, 6, 6}),
              kTfLiteOk);
  ...
  {
    ASSERT_EQ(
        m.Run(/*position=*/{1, 2}, /*k_sclie=*/{10, 10, 10, 10, 11, 11, 11, 11},
              /*v_sclie=*/{20, 20, 20, 20, 21, 21, 21, 21}),
        kTfLiteOk);
...
TEST_P(EKVCacheTest, FailsOnOutOfBoundPosition) {
  ExternalKVSingleOpModel m(
      ...
      {TensorType_INT64, {1}}, {TensorType_FLOAT32, {1, 1, 2, 2}},
      {TensorType_FLOAT32, {1, 1, 2, 2}}, GetParam());
  ASSERT_EQ(m.Run(/*position=*/{3}, /*k_sclie=*/{1, 2, 3, 4},
                  /*v_sclie=*/{1, 2, 3, 4}),
            kTfLiteError);
}
