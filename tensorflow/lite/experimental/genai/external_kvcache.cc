...
TfLiteStatus EKVCachePrepare(TfLiteContext* context, TfLiteNode* node) {
...
TfLiteStatus EKVCacheEval(TfLiteContext* context, TfLiteNode* node) {
  const TfLiteTensor* k_cache;
  const TfLiteTensor* v_cache;
  const TfLiteTensor* position;
  const TfLiteTensor* k_slice;
...
TfLiteRegistration* Register_EXTERNAL_KV_CACHE() {
  static TfLiteRegistration r = {nullptr, nullptr, llm::EKVCachePrepare,
                                 llm::EKVCacheEval};
  return &r;
}
