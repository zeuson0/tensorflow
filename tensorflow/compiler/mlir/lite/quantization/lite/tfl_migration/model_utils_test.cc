...
#include "tensorflow/compiler/mlir/lite/quantization/lite/tfl_migration/model_utils.h"
...
TEST(ModelUtilsTest, HasMinMax) {
  ...
  EXPECT_FALSE(mlir::lite::tfl_migration::HasMinMax(&tensor));
  tensor.quantization->max.push_back(1.5);
  EXPECT_TRUE(mlir::lite::tfl_migration::HasMinMax(&tensor));
}
