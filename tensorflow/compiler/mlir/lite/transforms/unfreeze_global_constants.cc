...
void UnfreezeMutableGlobalTensorsPass::runOnOperation() {
  ...
  if (target_global_tensor_ops.empty()) {
    VLOG(1) << "No mutable GlobalTensorOps found. "
               "UnfreezeMutableGlobalTensorsPass is a no-op.";
    return;
  }
  ...
  for (auto& it : global_tensor_uses) {
    GlobalTensorOp global_tensor = it.first;
    llvm::StringRef shared_name = global_tensor.getSymName();
    const auto element_type = TF::ResourceType::get(
        {mlir::cast<mlir::TensorType>(global_tensor.getType())},
    ...
    for (auto global_tensor_use : it.second) {
      auto func = global_tensor_use.func;
      llvm::BitVector args_to_erase(func.getNumArguments());
      args_to_erase.set(global_tensor_use.arg_index);
      ...
      auto arg = func.getArguments()[global_tensor_use.arg_index];
