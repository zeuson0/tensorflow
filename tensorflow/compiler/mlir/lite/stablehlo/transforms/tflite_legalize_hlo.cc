...
std::optional<bool> IsOrLegal(mhlo::OrOp op) {
  return !op.getType().getElementType().isInteger(1);
}

std::optional<bool> IsAndLegal(mhlo::AndOp op) {
  return !op.getType().getElementType().isInteger(1);
}

std::optional<bool> IsXorLegal(mhlo::XorOp op) {
  auto type = op.getType().getElementType();
  return !type.isInteger() || type.isInteger(1);
}
...
void LegalizeHloToTfLitePass::runOnOperation() {
  ...
  target.addDynamicallyLegalOp<mhlo::OrOp>(IsOrLegal);
  target.addDynamicallyLegalOp<mhlo::AndOp>(IsAndLegal);
  target.addDynamicallyLegalOp<mhlo::XorOp>(IsXorLegal);
